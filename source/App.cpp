...
bool App::Init()
{
	...
	if (!GetFont(FONT_SMALL)->Load("ui/font_century_gothic_bigx2.rtfont")) 
	{
		LogMsg("Can't load font 1");
		return false;
	}
	if (!GetFont(FONT_LARGE)->Load("ui/font_century_gothic_big.rtfont"))
	{
		LogMsg("Can't load font 2");
		return false;
	}
	if (!GetFont(FONT_GT)->Load("ui/font_century_gothic_bigx2.rtfont"))
	{
