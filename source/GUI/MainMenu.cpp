...
Entity * MainMenuCreate(Entity *pParentEnt)
{
	...
	*/
	...
	Entity* Menu = NULL;
	Menu = CreateOverlayEntity(pBG, "AboutMenu", "ui/large/game_title.rttex", 60, 0);
	Menu->GetVar("scale2d")->Set(CL_Vec2f(0.5, 0.5));

	pButtonEntity = CreateTextButtonEntity(pBG, "InputTest", x, y, "Play"); y += ySpacer;
	...
	pButtonEntity = CreateTextButtonEntity(pBG, "Debug", x, y, "Settings"); y += ySpacer;
	pButtonEntity->GetShared()->GetFunction("OnButtonSelected")->sig_function.connect(&MainMenuOnSelect);

	/*pButtonEntity = CreateTextButtonEntity(pBG, "About", x, y, "About"); y += ySpacer;
	pButtonEntity->GetShared()->GetFunction("OnButtonSelected")->sig_function.connect(&MainMenuOnSelect);*/

	SlideScreen(pBG, true);
	SlideScreen(Menu, true);
