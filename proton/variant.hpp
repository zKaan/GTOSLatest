...
class variantlist_t {
   ...
   public:
    ...
    bool serialize_from_mem_16(uint16_t* data, int* read = 0) { //robinson way cuz i dont want to refactor this yet
        uint16_t* p = data;
        uint16_t count = *(p++);
    ...
    bool serialize_from_mem_32(uint32_t* data, int* read = 0) { //robinson way cuz i dont want to refactor this yet
        uint32_t* p = data;
        uint32_t count = *(p++);
        for (int i = 0; i < count; i++) {
            uint32_t index = *(p++);
